...
Observacao: a estrategia de producao com refresh/revogacao continua fora do escopo da `BE-ARCH-01D`; a ADR da `BE-ARCH-01E1` registra a decisao, a `BE-ARCH-01E2` entregou a modelagem persistente e a `BE-ARCH-01E3` mitigou o gap backend de refresh, rotacao, cookie `HttpOnly` e logout server-side. Permanecem pendentes `BE-ARCH-01E4` para frontend com access em memoria/refresh silencioso, `BE-ARCH-01E5` para hardening operacional amplo de cookies/CORS/env e `BE-ARCH-01F` para auditoria/testes de eventos de autenticacao.
