...
Nota: a `BE-ARCH-01E2` implementou a primeira parte estrutural da ADR-002 com `UserSession` e a migration `20260430120000_add_user_session`; a `BE-ARCH-01E3` implementou a parte backend funcional com refresh token opaco, cookie `HttpOnly`, `POST /auth/refresh`, rotacao, revogacao por reuso e `POST /auth/logout`. A migracao frontend para o novo fluxo permanece pendente em `BE-ARCH-01E4`; hardening operacional amplo permanece pendente em `BE-ARCH-01E5`; auditoria formal permanece pendente em `BE-ARCH-01F`.
